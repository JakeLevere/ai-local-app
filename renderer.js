...
function setupEventListeners() { if (eventListenersAttached) { return; } console.log("Renderer: Attaching event listeners..."); const els = domElements;

// Helper function to add trackable event listeners
function addTrackedListener(element, event, handler, options = {}) {
    if (element) {
        element.addEventListener(event, handler, options);
        eventListenerCleanupFunctions.push(() => element.removeEventListener(event, handler, options));
    }
...
function setupIpcListeners() {
    ...
    window.electronAPI.on('main-process-warning', (warningMessage) => { console.warn('Renderer: Received warning from main process:', warningMessage); appendMessageToChatLog({ content: `Warning: ${warningMessage}` }, true); });
...
function cleanupEventListeners() {
    ...
    eventListenerCleanupFunctions.forEach(cleanup => cleanup());
    eventListenerCleanupFunctions = [];
    eventListenersAttached = false;
}

// Add beforeunload event to cleanup when the page is about to be unloaded
